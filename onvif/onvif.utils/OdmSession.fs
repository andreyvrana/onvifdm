...
//            use! response = request.AsyncGetResponse() |> Async.Map (fun x-> x:?>HttpWebResponse)
            use! response = request.AsyncGetResponse() |> Microsoft.FSharp.Control.Async.Map (fun x-> x:?>HttpWebResponse)
        ...
        member this.FtpDownload(uri:Uri):Async<Stream> = async{
            ...
            try
                ...
                do! async{
                    let! _response = (!request).AsyncGetResponse() |> Microsoft.FSharp.Control.Async.Map (fun x-> x:?>FtpWebResponse)
                    response := _response
...
//                request.Credentials <- cred_cache
            ...
            let! response = request.AsyncGetResponse() |> Microsoft.FSharp.Control.Async.Map (fun x-> x:?>HttpWebResponse)
            if response.StatusCode <> HttpStatusCode.OK then
                response.Close() 
                failwith "download failed"
