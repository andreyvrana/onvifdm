    ...
    type Nva = class
        ...
        static member GetControlInputConfigs(vand:IAnalyticsDeviceAsync, controlToken:string) = async{
            ...
            if configTokens.Length > 1 then
                let! allConfigs = vand.GetAnalyticsEngineInputs() |> Microsoft.FSharp.Control.Async.Map (SuppressNull ([||]))
                return [|
...
//            act.Load(inputTokens, -1)
        ...
        override this.Load((configTokens, receiverTokens)) = async{
            ...
            let! (allConfigs, allReceivers) = Async.Parallel(
                vand.GetAnalyticsEngineInputs() |> Microsoft.FSharp.Control.Async.Map (SuppressNull [||]),
                recv.GetReceivers() |> Microsoft.FSharp.Control.Async.Map (SuppressNull [||])
            )
